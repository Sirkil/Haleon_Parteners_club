...
//    paste the URL here.
// ═══════════════════════════════════════════════════════
const SHEETS_WEBHOOK = 'https://script.google.com/macros/s/AKfycbzy7ieNZHklk8TftWLPD6w_R7DV9eX0JRlPAq1SdhLc_MMtIOlnm2pirI-tjzGGHNZ-/exec';
...
const questions = [
  { q: 'Which vitamin is primarily synthesized by the body through sunlight exposure?',
    choices: ['Vitamin A','Vitamin C','Vitamin D','Vitamin B12'], correct: 2 },
  { q: 'What is the recommended daily water intake for an average adult?',
    choices: ['1 Liter','2 Liters','3 Liters','500 ml'], correct: 1 },
  { q: 'Which mineral is essential for strong bones and teeth?',
    choices: ['Iron','Potassium','Calcium','Magnesium'], correct: 2 },
  { q: 'How many hours of sleep are recommended for adults per night?',
    choices: ['4–5 hours','6–7 hours','7–9 hours','10–12 hours'], correct: 2 },
];
...
function switchTab(tab) {
  ...
  if (tab === 'home')    { showView('view-home');    updateHomeUI(); renderQuizCards(); }
...
document.addEventListener('touchstart', e => {
  ...
  if (!a || !['view-home','view-rewards','view-profile'].includes(a.id)) return;
...
document.addEventListener('touchend', e => {
  ...
  if (dx < 0 && currentTab < tabOrder.length - 1) switchTab(tabOrder[currentTab + 1]);
...
async function doLogout() {
  ...
  state.uid = null; state.user = null; state.score = 0;
  state.quizzesCompleted = 0; state.claimedBadges = []; state.answeredQuestions = [];
  answeredSet.clear();
...
function syncToSheets(uid, data) {
  ...
  const payload = {
    ...
    badges:            (data.claimedBadges || []).join(', '),
...
function openQuizScreen(qi) {
  ...
  document.getElementById('quiz-screen-q-num').textContent  = 'Question ' + (qi + 1);
...
async function answerQuizScreen(qi, ci) {
  if (answeredSet.has(qi)) return;
  const q = questions[qi];
  const btns = document.querySelectorAll('#quiz-screen-choices .quiz-screen-choice');
  btns.forEach((btn, i) => {
    btn.disabled = true;
  ...
  if (ci === q.correct) {
    ...
    document.getElementById('wrong-dialog-answer').textContent = 'Correct answer: ' + correctAnswer;
    setTimeout(() => {
...
function updateProfilePage() {
  ...
  document.getElementById('card-num-front').textContent      = state.user.memberId || '——';
